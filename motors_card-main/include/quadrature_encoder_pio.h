...
static inline void quadrature_encoder_get_count(PIO pio, uint sm_left, uint sm_right, int32_t *left_ticks, int32_t *right_ticks)
{
	pio->txf[sm_left] = 1;
    pio->txf[sm_right] = 1;
	while (pio_sm_is_rx_fifo_empty(pio, sm_left) && pio_sm_is_rx_fifo_empty(pio, sm_right))
		tight_loop_contents();
	*left_ticks = pio->rxf[sm_left];
    *right_ticks = pio->rxf[sm_right];
}
